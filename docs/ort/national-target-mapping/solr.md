...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: nationalTargetMapping })
  .map((json) => ({ json, protected: false }));
</script>

# National Target Mapping - Search

## Overview

Searches public National Target Mapping records of the Online Reporting Tool through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:nationalTarget7Mapping`. Combine criteria with `AND`, e.g. `schema_s:nationalTarget7Mapping AND text_EN_txt:biodiversity`. |

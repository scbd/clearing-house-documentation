...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: nationalReport })
  .map((json) => ({ json, protected: false }));
</script>

# National Report - Search

> **Draft** — pending review (REBUILD.md review gate).

## Overview

Searches public National Report records of the Online Reporting Tool through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:nationalReport7`. Combine criteria with `AND`, e.g. `schema_s:nationalReport7 AND text_EN_txt:biodiversity`. |

...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: nationalReportBinaryIndicatorData })
  .map((json) => ({ json, protected: false }));
</script>

# National Report Binary Indicator Data - Search

## Overview

Searches public National Report Binary Indicator Data records of the Online Reporting Tool through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:nationalReport7BinaryIndicatorData`. Combine criteria with `AND`, e.g. `schema_s:nationalReport7BinaryIndicatorData AND text_EN_txt:biodiversity`. |

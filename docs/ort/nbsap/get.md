...
const swaggerSpecs = [
  { json: getSpec({ apiUrl, realm, schema: nbsap }), protected: false },
];
</script>
...
Retrieves a single NBSAP record from the Online Reporting Tool by its unique identifier (`uid`). The `Accept` header controls the result format: `*` returns the record raw as submitted, `application/json` casts it to JSON.

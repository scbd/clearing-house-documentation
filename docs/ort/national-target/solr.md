...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: nationalTarget })
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:nationalTarget7`. Combine criteria with `AND`, e.g. `schema_s:nationalTarget7 AND text_EN_txt:biodiversity`. |

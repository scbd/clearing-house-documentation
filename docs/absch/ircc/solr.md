...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: ircc })
  .map((json) => ({ json, protected: false }));
</script>

# Internationally Recognized Certificate of Compliance - Search

> **Draft** — pending review (REBUILD.md review gate).

## Overview

Searches public Internationally Recognized Certificate of Compliance records of the ABS Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:absPermit`. Combine criteria with `AND`, e.g. `schema_s:absPermit AND text_EN_txt:biodiversity`. |

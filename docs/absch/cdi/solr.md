...
import cdi from "@/swagger/schemas/abs/cdi"
import { solrSpecs } from "@/swagger/generators/solr"
import { useClearingHouse } from "@/utils/composables"

const { apiUrl, realm } = useClearingHouse()

const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: cdi })
...
Searches public Capacity Development Initiative records of the ABS Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:capacityBuildingInitiative`. Combine criteria with `AND`, e.g. `schema_s:capacityBuildingInitiative AND text_EN_txt:biodiversity`. |

...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: cpc })
  .map((json) => ({ json, protected: false }));
</script>

# Checkpoint Communique - Search

## Overview

Searches public Checkpoint Communique records of the ABS Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:absCheckpointCommunique`. Combine criteria with `AND`, e.g. `schema_s:absCheckpointCommunique AND text_EN_txt:biodiversity`. |

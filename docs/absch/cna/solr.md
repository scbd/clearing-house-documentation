...
import cna from "@/swagger/schemas/abs/cna"
import { solrSpecs } from "@/swagger/generators/solr"
import { useClearingHouse } from "@/utils/composables"

const { apiUrl, realm } = useClearingHouse()

const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: cna })
  .map((json) => ({ json, protected: false }));
</script>

# Competent National Authority - Search

> **Draft** — pending review (REBUILD.md review gate).

## Overview

Searches public Competent National Authority records of the ABS Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:authority`. Combine criteria with `AND`, e.g. `schema_s:authority AND text_EN_txt:biodiversity`. |

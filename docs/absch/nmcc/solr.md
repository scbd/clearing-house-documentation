...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: nmcc })
  .map((json) => ({ json, protected: false }));
</script>

# National Model Contractual Clause - Search

## Overview

Searches public National Model Contractual Clause records of the ABS Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:absNationalModelContractualClause`. Combine criteria with `AND`, e.g. `schema_s:absNationalModelContractualClause AND text_EN_txt:biodiversity`. |

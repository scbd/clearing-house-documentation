...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: cpp })
  .map((json) => ({ json, protected: false }));
</script>

# Community Protocol - Search

> **Draft** — pending review (REBUILD.md review gate).

## Overview

Searches public Community Protocol records of the ABS Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:communityProtocol`. Combine criteria with `AND`, e.g. `schema_s:communityProtocol AND text_EN_txt:biodiversity`. |

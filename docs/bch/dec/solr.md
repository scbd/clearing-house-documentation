...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: dec })
...
Searches public Decision records of the Biosafety Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:biosafetyDecision`. Combine criteria with `AND`, e.g. `schema_s:biosafetyDecision AND text_EN_txt:biodiversity`. |

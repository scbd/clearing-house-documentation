...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: vlr })
  .map((json) => ({ json, protected: false }));
</script>

# Virtual Library Resource - Search

## Overview

Searches public Virtual Library Resource records of the Biosafety Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:resource`. Combine criteria with `AND`, e.g. `schema_s:resource AND text_EN_txt:biodiversity`. |

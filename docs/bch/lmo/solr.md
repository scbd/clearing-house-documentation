...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: lmo })
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:modifiedOrganism`. Combine criteria with `AND`, e.g. `schema_s:modifiedOrganism AND text_EN_txt:biodiversity`. |

...
const swaggerSpecs = solrSpecs({ apiUrl, realm, schema: ndb })
  .map((json) => ({ json, protected: false }));
</script>

# National Website or Database - Search

> **Draft** — pending review (REBUILD.md review gate).

## Overview

Searches public National Website or Database records of the Biosafety Clearing-House through the Apache Solr index. The playground below shows the common query patterns: all records of the type, filtered by country, by free text, by region, and with combined sub-filters.
...
| --- | --- |
| `q` | The main query, e.g. `schema_s:database`. Combine criteria with `AND`, e.g. `schema_s:database AND text_EN_txt:biodiversity`. |

...
| general (drafts, attachments, files) | A B C O | drafted 2026-08-05. 8 topics × 4 apps: the v2013 publishing chain (drafts, draft-locks, draft-securities, document-securities, document-validations, documents-versions) is **unlisted** (no sidebar; carries `<LegacyNotice>`; linked from every create/update page) per ADR 0002; document-attachments + temporary-file are **listed** under a "Files & Attachments" sidebar section. Generator `general.ts` + `general-topic.vue` render each topic's operations from a compact manifest. Flags: legacy specs use **generic `object` request/response bodies** (faithful shapes deprioritized for a phased-out endpoint set — review if fuller docs are wanted); HEAD used for the `exists` operations (archive method wasn't explicit); `file-upload` "upload to user-defined URL" (PUT /) operation omitted |
